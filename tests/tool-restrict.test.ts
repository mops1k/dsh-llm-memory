...
import { MEMORY_FULL_ONLY_TOOL_NAMES } from '../src/dsh/tools'
import { apply, Config, inject, name } from '../src/tool-restrict'

describe('dsh-llm-memory tool restriction', () => {
  it('exposes a scoped plugin that denies the exact full-only set by default', () => {
    expect(name).toBe('dsh-llm-memory-tool-restrict')
    ...
    expect(restrict).toHaveBeenCalledWith({ deny: [...MEMORY_FULL_ONLY_TOOL_NAMES] })
  })

  it('accepts an explicit deny list and rejects unknown tools', () => {
    expect(Config({ deny: ['llm_memory_lint'] }).deny).toEqual(['llm_memory_lint'])
    expect(() => Config({ deny: ['llm_memory_recall'] })).toThrow()
  })
